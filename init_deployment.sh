...
# next line works on OS-X
#MONGO_VERSION=`m available --stable | grep '3.2.' | head -1 | awk '{print $2}' | strings`
# this one works on Amazon/RHEL
#MONGO_VERSION=`m available --stable | grep --color=never '3.2.' | \
#tr -s '[:blank:]' | head -1 | awk -F'\033' '{print $1}' | tr -d '\n'`

# m changes the output too much to auto-detect right now
# just hard code version for testing here
MONGO_VERSION=3.2.17
echo "using MongoDB version $MONGO_VERSION"
MONGO_PATH=`m path`
CURRENT_VERSION=`$MONGO_PATH/mongod --version | head -1 | awk -Fv '{print $3}'`
if ! [ "$CURRENT_VERSION" == "$MONGO_VERSION" ]; then
    echo "m use $MONGO_VERSION"
